...
Test of creating a 3D vector word map using Tensorflow's prebuilt
...
print("[{0}]Downloading dependencies: punkt and stopwords".format(datetime.datetime.now().strftime("%H:%M.%S")))

nltk.download("punkt")
nltk.download("stopwords")

print("[{0}]Searching home directory for files".format(datetime.datetime.now().strftime("%H:%M.%S")))
...
print("[{}]Transform Fitting. May take about {} minutes".format(datetime.datetime.now().strftime("%H:%M.%S"), len(wap2vec.wv.vocab)/1000))
...
slice = points[
    (0 <= points.x) &
    (points.x <= len(points.x)) &
    (0 <= points.y) &
    (points.y <= len(points.y))
    ]

for i, point in slice.iterrows():
    ax.text(x=point.x + 0.005, y=point.y + 0.005, z=point.z + 0.005, label="Hello", fontsize=11,)
"""
plt.show()
