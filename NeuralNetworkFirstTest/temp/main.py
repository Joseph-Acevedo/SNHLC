...
#from GraphicsTest import VisualizeNetwork
...
class NeuralNetwork():
    ...
    def get_activation(self, neuronID, error):
        ...
        activation = (np.interp(output,[0,1],[0,number_of_activations - 1])).astype(int) + 1
    ...
    '''def train(self, network_input, target, guess, errormatrix, network_output, iterations, stratified = False):
        for i in range(iterations):
            length = len(self.layer_dict)
            delta = []
            error = []
            for l in range(length, 0, -1):
                ...
                if(l != length):
                    ...
                    j = 0
                    for n in np.nditer(layer.activations):
                        error = nextlayer.weights.T.dot(delta.T)
                        delta = error * layer.derivatives[layer.activations[j]](nextlayer.last_inputs)
                        if(l != 1):
                            adjustment = layer.last_inputs.dot(delta.T)
                        else:
                            error = delta.dot(layer.weights.T)
                            delta = error.T * layer.derivatives[layer.activations[j]](nextlayer.last_inputs)
                            adjustment = network_input.T.dot(delta)
                        layer.weights[:,j] += adjustment
                        j += 1
                else:
                    if(stratified):
                        error = network_output - np.linalg.norm(target - guess)
                    else:
                        error = errormatrix
                    j = 0
                    for n in np.nditer(layer.activations):
                        delta = error * layer.derivatives[layer.activations[j]](network_output)
                        adjustment = network_output * delta
                        layer.weights[:,j] += adjustment
                        j += 1'''
...
if __name__ == "__main__":
    ...
    n2_layer3_number_of_neurons = 15
